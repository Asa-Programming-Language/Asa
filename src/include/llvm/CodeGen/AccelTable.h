...
namespace llvm {
    ...
    class AccelTable : public AccelTableBase {
    public:
        AccelTable()
            : AccelTableBase(DataT::hash) {}
    ...
    public:
        ...
        DWARF5AccelTableData(const uint64_t DieOffset,
            ...
            const bool IsTU)
            : OffsetVal(DieOffset), ParentOffset(DefiningParentOffset), DieTag(DieTag), AbbrevNumber(0), IsTU(IsTU), UnitID(UnitID) {}
    ...
    LLVM_ABI void
    emitAppleAccelTableImpl(AsmPrinter* Asm, AccelTableBase& Contents, StringRef Prefix, const MCSymbol* SecBegin, ArrayRef<AppleAccelTableData::Atom> Atoms);
    ...
    LLVM_ABI void
    emitDWARF5AccelTable(AsmPrinter* Asm, DWARF5AccelTable& Contents, const DwarfDebug& DD, ArrayRef<std::unique_ptr<DwarfCompileUnit>> CUs);
