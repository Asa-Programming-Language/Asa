...
namespace llvm {
	...
	namespace IntervalMapImpl {
		...
		public:
			...
			unsigned findFrom(unsigned i, unsigned Size, KeyT x) const
			{
				...
				while (i != Size && Traits::stopLess(stop(i), x))
					++i;
				return i;
			}
	...
	public:
		...
		KeyT stop() const
		{
			...
			return !branched() ? rootLeaf().stop(rootSize - 1) : rootBranch().stop(rootSize - 1);
		}
		...
		ValT lookup(KeyT x, ValT NotFound = ValT()) const
		{
			if (empty() || Traits::startLess(x, start()) || Traits::stopLess(stop(), x))
				return NotFound;
			return branched() ? treeSafeLookup(x, NotFound) : rootLeaf().safeLookup(x, NotFound);
		}
	...
	protected:
		...
		KeyT& unsafeStart() const
		{
			...
			return branched() ? path.leaf<Leaf>().start(path.leafOffset()) : path.leaf<RootLeaf>().start(path.leafOffset());
		}
		...
		{
			assert(valid() && "Cannot access invalid iterator");
			return branched() ? path.leaf<Leaf>().stop(path.leafOffset()) : path.leaf<RootLeaf>().stop(path.leafOffset());
		}
		...
		{
			assert(valid() && "Cannot access invalid iterator");
			return branched() ? path.leaf<Leaf>().value(path.leafOffset()) : path.leaf<RootLeaf>().value(path.leafOffset());
		}
	...
	class IntervalMap<KeyT, ValT, N, Traits>::iterator : public const_iterator {
		...
		explicit iterator(IntervalMap& map)
			: const_iterator(map) {}
